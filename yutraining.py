...
import pandas as pd
import numpy as np
import yulib.yudata_preprocessing as dpp
...
if __name__ == "__main__":
    ...
    parser.add_argument(key_word, required=False, default='./Mydata_handsup.csv')
    input = parser.parse_args().dataset

    # Loading training data
    #pose1是举手，pose2是平放手
    try:
        raw_data_pose1 = pd.read_csv(input, header=0)
        raw_data_pose2=pd.read_csv('./Mydata_handsdown.csv')
    except:
        print("Dataset not exists.")
    # X: input, Y: output
    dataset1 = raw_data_pose1.values
    dataset2 = raw_data_pose2.values
