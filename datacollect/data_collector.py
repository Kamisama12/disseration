...
def data_worker(mode, seconds, filepath):
	...
	while collect:
		if (time.time() - start_time < seconds):
			...
			myo_cols = ["Channel_1", "Channel_2", "Channel_3", "Channel_4", "Channel_5", "Channel_6", "Channel_7", "Channel_8","gesture"]
			gesture=np.linspace(1,1,len(myo_data))#采集不同手势的时候更改数字
			myo_data=np.column_stack((myo_data,gesture))
...
if __name__ == '__main__':
	...
	seconds = 10
	file_name = str(seconds)+"gesture_1_"+"_test_emg.csv"
