...
class Yzh_CNN_Model(Classifier):
	...
	def __init__(self):
		Classifier.__init__(self)
		self.model=MyLearningModelPredict.CNN(input_shape = (12, 1, 8), classes = 2)
		self.model.summary()
		self.model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
		self.chenkpoint = "D:\\bristolUNI\\dissertation\\code\\EMG\\yuzhaohao.h5"
	...
	def classify(self, emg):
		...
		x = np.array(emg).reshape(1,12,1,8)
		print(x)
		...
		return np.argmax(result,axis=1)[0]#返回计算概率最大的值对应的索引，引出[0]，否则narray数据类型是unhashable的
		'''
		握拳是姿势0，张手是姿势1
		'''
...
q = multiprocessing.Queue()#multiprocess.Queue是多进程通讯用的队列，因为不同进程之间不共享全局变量，用来存不同进程中的数据
m = multiprocessing.Queue()#用来存储一帧需要预测数据
def My_worker(q):#获取EMG数据
	def on_press(key):
...
# 释放键盘时回调的函数
	...
	m = MyoClassifier(Yzh_CNN_Model(), mode=emg_mode.PREPROCESSED,Networkmode=True)
	hnd = EMGHandler(m)#EMGHandler中的emg成员直接就是存储了实时的emg数据
...
if __name__ == "__main__":
	...
	first = multiprocessing.Process(target=My_worker, args=(q,))
